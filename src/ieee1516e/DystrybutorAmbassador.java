...
public class DystrybutorAmbassador extends NullFederateAmbassador
{
	...
	public DystrybutorAmbassador( Dystrybutor federate )
	{
	...
	public void announceSynchronizationPoint( String label, byte[] tag )
	{
		...
		if( label.equals(Dystrybutor.READY_TO_RUN) )
	    ...
	    throws FederateInternalError
	{
		...
		for( ParameterHandle parameter : theParameters.keySet() )
		{
			...
			if( parameter.equals(federate.typ_zgloszeniaHandle) )
			{
				...
				if (decodetyp_zgloszenia(theParameters.get(parameter)) == 1) {
					typzgloszenia = 1;
			...
			else if( parameter.equals(federate.czas_wykonaniaHandle) )
			{
				...
				for(int i = 0; i<federate.ilosc_dystrybutorow; i++) {
					if (federate.dystrybutory[i].getczywolny() && typzgloszenia == 1) {
						federate.dystrybutory[i].dostalem();
						federate.dystrybutory[i].ilelac(decodeczas_wykonania(theParameters.get(parameter)));
						break;
			...
			else if( parameter.equals(federate.nr_samochoduHandle) )
			{
				...
				for(int i = 0; i < federate.ilosc_dystrybutorow; i++) {
					if (federate.dystrybutory[i].getczywolny() && typzgloszenia == 1) {
						federate.dystrybutory[i].obslugiwanysamochod = decodenrsamochodu(theParameters.get(parameter));
						break;
						//federate.rozpocznijtankowanie[i] = true;
					}
				}
