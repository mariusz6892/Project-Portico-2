...
public class Kasa
{
	...
	public Kasajeden kasa = new Kasajeden();
	...
	private KasaAmbassador fedamb;  // created when we connect
	...
	{
		System.out.println( "Kasa   : " + message );
	}
	...
	public void runFederate( String federateName ) throws Exception
	{
		...
		// connect
		log( "Connecting..." );
		fedamb = new KasaAmbassador( this );
		...
		for( int i = 0; i < ITERATIONS; i++ )
		{
			...
			// 9.2 send an interaction
				if (kasa.getczywolny() == true && dlugosc_kolejki > 0 && dostalem == true) {
					sendInteraction(true,1,kasa.obslugiwanysamochod);
					dlugosc_kolejki--;
					//updateAttributeValues( objectHandle );
					kasa.zajmij();
				}
			//for( int j=0; j<ilosc_dystrybutorow; j++) 
				...
				if (kasa.getczywolny() == false) kasa.placenie();
			
				if (kasa.styknie() && kasa.getczywolny() == false) {
					kasa.koniecplacenia();
					dostalem = false;
					sendInteraction(false,1,kasa.obslugiwanysamochod);
				}
			advanceTime( 1.0 );
	...
	public static void main( String[] args )
	{
		...
		try
		{
			...
			new Kasa().runFederate( federateName );
		}
