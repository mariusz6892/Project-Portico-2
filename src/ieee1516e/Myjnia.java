...
public class Myjnia
{
	...
	public Myjniajeden myjnia = new Myjniajeden();
	public boolean dostalem = false;
	
	//----------------------------------------------------------
	//                   INSTANCE VARIABLES
	//----------------------------------------------------------
	private RTIambassador rtiamb;
	private MyjniaAmbassador fedamb;  // created when we connect
	...
	{
		System.out.println( "Myjnia   : " + message );
	}
	...
	public void runFederate( String federateName ) throws Exception
	{
		...
		// connect
		log( "Connecting..." );
		fedamb = new MyjniaAmbassador( this );
		...
		for( int i = 0; i < ITERATIONS; i++ )
		{
			...
			// 9.2 send an interaction
				if (myjnia.getczywolny() == true && dlugosc_kolejki > 0 && dostalem == true) {
					sendInteraction(true,1,myjnia.obslugiwanysamochod);
					dlugosc_kolejki--;
					//updateAttributeValues( objectHandle );
					myjnia.zajmij();
				}
			//for( int j=0; j<ilosc_dystrybutorow; j++) 
				...
				if (myjnia.getczywolny() == false) myjnia.mycie();
			
				if (myjnia.styknie() && myjnia.getczywolny() == false) {
					myjnia.koniecmycia();
					dostalem = false;
					sendInteraction(false,1,myjnia.obslugiwanysamochod);
				}
			advanceTime( 1.0 );
	...
	public static void main( String[] args )
	{
		...
		try
		{
			...
			new Myjnia().runFederate( federateName );
		}
